...
void imu_setup(){
    ...
    recv = readRegister(IMU_ADDR,WHO_AM_I);
    
    if (recv != 0b01101001){
        while(1){
            switch(counter){case 50:LATAbits.LATA4=!LATAbits.LATA4;counter=0;} 
            counter++;
        }
    }

    setRegister(IMU_ADDR,CTRL1_XL,0b10000010); // 1.66kHz output data rate, +/-2g sensitivity, 100Hz filter
    
    setRegister(IMU_ADDR,CTRL2_G,0b10001000);  // 1.66kHz output data rate, 1000 dps
    
    setRegister(IMU_ADDR,CTRL3_C,0b00000100);  // read from multiple registers in a row without specifying every register location
